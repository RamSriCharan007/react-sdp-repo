...
export default function ViewBookingsByCustomer() {
  const [data, setData] = useState({ bookings: [], customerUsername: '', loading: true });
  ...
  useEffect(() => {
    ...
    // Use a microtask to defer state update and avoid cascading render warning
  ...
  const handleDeleteBooking = (bookingId) => {
    if (window.confirm(`Are you sure you want to cancel booking ID ${bookingId}? This action cannot be undone.`)) {
      ...
      const customerBookings = updatedBookings.filter(booking => booking.customerUsername === data.customerUsername);
      setData(prev => ({ ...prev, bookings: customerBookings }));
  ...
  return (
    <div style={{ padding: '20px' }}>
      <h2>My Bookings</h2>
      <p style={{ fontSize: '14px', color: '#666' }}>Customer: <strong>{data.customerUsername}</strong></p>
        ...
        <div style={{ marginTop: '30px', padding: '20px', backgroundColor: '#f5f5f5', borderRadius: '8px' }}>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <h4 style={{ margin: 0, fontSize: '24px', color: '#007bff' }}>{data.bookings.length}</h4>
            </div>
            <div style={{
              backgroundColor: '#fff',
              padding: '15px',
            ...
            <div style={{
              ...
              <h4 style={{ margin: 0, fontSize: '24px', color: '#ffc107' }}>
                {data.bookings.filter(b => new Date(b.endDate) >= new Date()).length}
              </h4>
            </div>
          </div>
        </div>
